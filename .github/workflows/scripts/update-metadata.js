...
const metadataFile = path.join(process.cwd(), 'metadata.json');
    ...
    try {
        ...
        for (const file of files) {
            if (file.endsWith('.mdx')) {
                ...
                const existingIndex = metadataJson.metadata.findIndex(entry => entry.__outstatic.path === newMetadataEntry.__outstatic.path);
                if (existingIndex !== -1) {
                    metadataJson.metadata[existingIndex] = newMetadataEntry; // Update existing entry
                    console.log(`Updated metadata for: ${newMetadataEntry.__outstatic.path}`);
                } else {
                    metadataJson.metadata.push(newMetadataEntry); // Add new entry
                    console.log(`Added metadata for: ${newMetadataEntry.__outstatic.path}`);
                }
            }
        }
